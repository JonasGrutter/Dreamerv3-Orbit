...
def simulate(
    ...
    while (steps and step < steps) or (episodes and episode < episodes):
        ...
        if done.any():
            if ORBIT:
                indices = np.nonzero(done)[0] # Nonzero gives a tuple
                obs = envs.reset_idx(indices)
                # Cache new episodes
                for ids in indices:
                    # Define the transition
                    transition = obs[ids].copy()
        ...
        # step envs
        if ORBIT:
            obs, reward, done, extras = envs.step(action)
        else:
        ...
        if done.any():
            ...
            for i in indices:
                if ORBIT:
                    save_episodes(directory, {envs.unique_indices[i]: cache[envs.unique_indices[i]]})
                    length = len(cache[envs.unique_indices[i]]["reward"]) - 1
                    score = float(np.array(cache[envs.unique_indices[i]]["reward"]).sum())
                    #video = cache[envs.unique_indices[i]]["image"]
